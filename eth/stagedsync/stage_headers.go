...
func verifyAndSaveNewPoSHeader(
	...
	currentHeadHash := rawdb.ReadHeadHeaderHash(tx)
	sideChain := currentHeadHash != header.ParentHash

	extendForkInMemory := cfg.memoryOverlay && (cfg.forkValidator.ExtendingForkHeadHash() == (common.Hash{}) || header.ParentHash == cfg.forkValidator.ExtendingForkHeadHash())
	if sideChain || extendForkInMemory {
		status, latestValidHash, validationError, criticalError := cfg.forkValidator.ValidatePayload(tx, header, body, extendForkInMemory)
